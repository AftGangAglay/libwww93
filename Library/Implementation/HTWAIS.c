...
static void output_text_record(
	...
	if(binary) {
		(*target->isa->put_block)(
				target, record->DocumentText->bytes,
...
void display_search_response(
	...
	sprintf(
			...
			response->NumberOfRecordsReturned == 1 ? "" : "s", keywords);
...
*/
				if(TRACE) {
					fprintf(
							...
							(long int) (info->DocHeaders[k]->Lines), headline);
				}
				...
				if(archie) {
					...
					if(docname) {
						...
						sprintf(
								line, "%s/%s/%d/%s",        /* W3 address */
								dbname, head->Types ? head->Types[0] : "TEXT",
								(int) (head->DocumentLength), docname);
						HTStartAnchor(
								target, NULL, ((head->Types) &&
		...
		if(info->Text != 0) {
			...
			while(info->Text[k] != 0) {
				...
				PUTS("\nText record\n");
				output_text_record(
						(HTStream*) target, info->Text[k++], false, false);
			}
		}
		if(info->Headlines != 0) {
			k = 0;
	...
	if(server_name[0] == 0) {
		...
		if((connection = connect_to_server(server_name, atoi(service))) ==
		   NULL) {
			...
			free(names);
			return HTLoadError(
					sink, 500, "Can't open connection to WAIS server");
		}
	}
...
*/
	...
	if(key && !*key) {                /* I N D E X */
		...
		*/
		...
		if(TRACE) {
			fprintf(
					stderr, "HTWAIS: Search for `%s' in `%s'\n", keywords,
					wais_database);
		}
		if(NULL == generate_search_apdu(
				request_message + HEADER_LENGTH, &request_buffer_length,
				keywords, wais_database, NULL, MAXDOCS)) {
			panic("request too large");
		}


		if(!interpret_message(
				request_message, MAX_MESSAGE_LEN - request_buffer_length,
			...
			readSearchResponseAPDU(
					&query_response, response_message + HEADER_LENGTH);
			display_search_response(
					target, query_response, wais_database, keywords);
		...
		format_in = !strcmp(doctype, "WSRC") ? HTAtom_for(
				"application/x-wais-source") : !strcmp(doctype, "TEXT")
											   ? HTAtom_for("text/plain")
											   : !strcmp(doctype, "HTML")
												 ? HTAtom_for("text/html")
												 : !strcmp(doctype, "GIF")
												   ? HTAtom_for("image/gif")
												   : HTAtom_for(
										"application/octet-stream");
		binary = 0 != strcmp(doctype, "WSRC") && 0 != strcmp(doctype, "TEXT") &&
...
*/
		for(count = 0; count * CHARS_PER_PAGE < document_length; count++) {
			...
			if(0 == generate_retrieval_apdu(
					request_message + HEADER_LENGTH, &request_buffer_length,
					docid, CT_byte, count * CHARS_PER_PAGE,
					MIN((count + 1) * CHARS_PER_PAGE, document_length), type,
					wais_database)) {
				panic("request too long");
			}

			/*	Actually do the transaction given by request_message */
			if(0 == interpret_message(
					request_message, MAX_MESSAGE_LEN - request_buffer_length,
					response_message, MAX_MESSAGE_LEN, connection,
					false /* true verbose */
									 )) {
			...
			*/
			readSearchResponseAPDU(
					&retrieval_response, response_message + HEADER_LENGTH);
			...
			if(NULL == ((WAISSearchResponse*) retrieval_response
				...
				output_text_record(
						target, ((WAISSearchResponse*) retrieval_response
								->DatabaseDiagnosticRecords)->Text[0], false,
						binary);
