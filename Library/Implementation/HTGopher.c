...
*/
	switch(gtype) {
		...
		case GOPHER_HTML :
			HTParseSocket(
					WWW_HTML, format_out, anAnchor, s, sink);
			break;
		...
		case GOPHER_IMAGE:
			...
			break;
		case GOPHER_MENU :
		case GOPHER_INDEX : target = HTML_new(anAnchor, format_out, sink);
			targetClass = *target->isa;
			parse_menu(arg, anAnchor);
			break;

		case GOPHER_CSO: target = HTML_new(anAnchor, format_out, sink);
			targetClass = *target->isa;
		...
		case GOPHER_SOUND :
			HTParseSocket(
					WWW_AUDIO, format_out, anAnchor, s, sink);
			break;
