...
void HTFormatInit(void) {
	...
	if(getenv("DISPLAY")) {    /* Must have X11 */
		HTSetPresentation(
				"application/postscript", "ghostview %s", 1.0, 3.0, 0.0);
	...
	HTSetConversion(
			"application/x-wais-source", "*", HTWSRCConvert, 1.0, 0.0, 0.0);
	...
	HTSetConversion(
			"application/octet-stream", "www/present", HTSaveLocally, 0.1, 0.0,
			0.0);
...
void HTFileInit(
		void) {    /* 	Suffix     Contenet-Type   Content-Encoding    Quality  */

	HTSetSuffix(
			".ai", "application/postscript", "8bit",
			0.5);    /* Adobe illustator */
	HTSetSuffix(".au", "audio/basic", "binary", 1.0);
	HTSetSuffix(
			".mime", "www/mime", "8bit",
			1.0); /* Internal -- MIME is not recursive? */
