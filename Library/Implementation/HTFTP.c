...
static int response(char* cmd) {
	...
	do {
		...
		for(;;) {
			...
			if(*(p - 1) < 0) {
				if(TRACE) {
					fprintf(
							stderr, "Error on rx: closing socket %d\n",
							control->socket);
				}
	...
	if(result == 421) {
		if(TRACE) {
			fprintf(
					stderr, "FTP: They close so we close socket %d\n",
					control->socket);
		}
		close_connection(control);
		return -1;
...

	{
		...
		for(scan = connections; scan; scan = scan->next) {
			if(sin->sin_addr.s_addr == scan->addr) {
				...
				if(TRACE) {
					fprintf(
							stderr, "FTP: Existing connection is %d.%d.%d.%d\n",
							(int) *((unsigned char*) (&scan->addr) + 0),
							(int) *((unsigned char*) (&scan->addr) + 1),
							(int) *((unsigned char*) (&scan->addr) + 2),
							(int) *((unsigned char*) (&scan->addr) + 3));
...
*/
	{
		...
		status = connect(
				con->socket, (struct sockaddr*) &soc_address,
				sizeof(soc_address));
		if(status < 0) {
		...
		connections = con;
		HTInitInput(
				con->socket);/* Initialise buffering for contron connection */
...
*/
	...
	if(TRACE) {
		fprintf(
				stderr, "FTP: Opened master socket number %d\n", new_socket);
	}
...
*/
	...
	sprintf(
			port_command, "PORT %d,%d,%d,%d,%d,%d%c%c",
...
static int read_directory(
...

	{
		...
		for(c = 0; c != (char) EOF;)   /* For each entry in the directory */
		{
			...
			if(c == (char) EOF &&
			   chunk->size == 1) {  /* 1 means empty: includes terminating 0 */
				break;
			}
			if(TRACE) {
				fprintf(
...
int HTFTPLoad(
	...
	for(retry = 0; retry < 2;
			retry++) {    /* For timed out/broken connections */
...
*/
	...
	if(isDirectory) {
		...
		HTParseSocket(format, format_out, anchor, data_soc, sink);
