...
typedef HTStream* HTConverter(HTPresentation* pres, HTParentAnchor* anchor, HTStream* sink);
 ...
 */
HTStream* HTStreamStack 		(HTFormat format_in, HTFormat format_out, HTStream * stream_out,
 ...
 */
float HTStackValue (HTFormat format_in, HTFormat rep_out,
										 float initial_value,
										 long int length);
...
HTCopyNoCR: Copy a socket to a stream, stripping '\r' characters.
 ...
 */
int HTParseSocket
		(HTFormat format_in, HTFormat format_out, HTParentAnchor * anchor,
		int file_number,
		HTStream* sink);
 ...
 */
extern HTBool HTOutputSource;     /* Flag: shortcut parser */
#endif
